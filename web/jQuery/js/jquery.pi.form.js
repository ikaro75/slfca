        ...
        $.ajax(
                {
                    ...
                    success: function(data) {
                        ...
                        if ($.fn.form.options.showRelationships == "true" && $("#formTab_" + formSuffix).attr("modo") == 'update') {
                            if ($(xml).find("configuracion_forma")) {
                                ...
                                oTabsForaneos.each(function() {
                                    if ($($(this).next().next()).text() == "1"  ) { // Solo trae las formas foraneas cuyo tipo de presentación es el de pestañas
                        ...
                        if (sPermiso.indexOf("3") == -1 && $("#formTab_" + formSuffix).attr("modo") == 'update') {
                            //Aqui se debe de quita rel botón de guardar
                        ...
                        //Fix de la Â que aparece al inyectar html en la forma - Inicia
                        $("#divFormGeneral_" + formSuffix).html($("#divFormGeneral_" + formSuffix).html().replace("\u00C2",""));         
                        //Fix de la Â que aparece al inyectar html - Finaliza
                        ...
                        function processXml(data) {
                            ...
                            if (error.length > 0) {
                                if ($("#form_149_729_0").length>0) {
                                    $("#id_tienda").removeAttr("disabled");
                                }
                                
                                if ($("#form_150_733_0").length>0) {
                                    $("#rpu").removeAttr("disabled");
                                }
                            ...
                            //Mete el número de cuestionario_participante en cache para el cuestionario de supervisión
                            if ($("#form_149_729_0").length>0) {
                                //Busca el id_tienda del cuestionario de supervisión
                                $.ajax({
                                    url: "control?$cmd=plain&$cf=729&$ta=select&$w=clave_cuestionario_participante="+sResultado,
                                    dataType: ($.browser.msie) ? "text" : "xml",
                                    success: function(data) {
                                        if (typeof data == "string") {
                                        ...
                                        if ($(xmlIdTienda).find("error").length > 0) {
                                            alert($(xmlIdTienda).find("error").text());
                                        }

                                        $("#_cache_").val(sResultado+ "_" + $(xmlIdTienda).find("id_tienda")[0].childNodes[0].data);
                            ...
                            //Mete el número de cuestionario_participante en cache para la encuesta de opinión
                            if ($("#form_150_733_0").length>0) {
                                //Busca el rpu de la encuesta de opinion
                                $.ajax({
                                    url: "control?$cmd=plain&$cf=733&$ta=select&$w=fide_cuestionario_participante.clave_cuestionario_participante="+sResultado,
                                    dataType: ($.browser.msie) ? "text" : "xml",
                                    success: function(data) {
                                        if (typeof data == "string") {
                                            xmlRPUEncuesta = new ActiveXObject("Microsoft.XMLDOM");
                                            xmlRPUEncuesta.async = false;
                                            xmlRPUEncuesta.validateOnParse = "true";
                                            xmlRPUEncuesta.loadXML(data);
                                            if (xmlRPUEncuesta.parseError.errorCode > 0) {
                                                alert("Error de compilaci\u00f3n xml:" + xmlRPUEncuesta.parseError.errorCode + "\nParse reason:" + xmlRPUEncuesta.parseError.reason + "\nLinea:" + xmlRPUEncuesta.parseError.line);
                                            }
                                        }
                                        else {
                                            xmlRPUEncuesta = data;
                                        ...
                                        if ($(xmlRPUEncuesta).find("error").length > 0) {
                                            alert($(xmlRPUEncuesta).find("error").text());
                                        }

                                        $("#_cache_").val(sResultado+ "_" + $(xmlRPUEncuesta).find("rpu")[0].childNodes[0].data);
    ...
    $.fn.form.handleForm = function(xml) {
        ...
        oCampos.each(function() {
            ...
            if ($.fn.form.options.modo=="lookup" && bBusquedaPorRango) {
                sRenglon+= "§" + sRenglon.split("§")[sRenglon.split("§").length-1].replace(/\$inicial/gi,"$final").replace("(inicial)","(final)") + "§" ;
            } else {
                sRenglon+= "§";
            }
            tabIndex++;
