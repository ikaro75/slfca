(function($) {
    $.fn.seleccionamesa = function(opc){

        $.fn.seleccionamesa.settings = {
            height:"300"
        };
        ...
        function setGUI (obj) {
                  $.ajax({
                  url: "table_picker.jsp?$empresa=" + $("#_enterprise_").val() + "&$empleado=" + $("#_ce_").val() + "&$ta=open",
                  ...
                  success: function(data) {
                      ...
                      var textoBoton="";
                      if ($(xml).find("input").length==0) {
                          textoBoton="Cerrar";
                      } else {
                          textoBoton="Seleccionar mesas a atender";
                      }
                          
                      obj.append("<div id='tablePickerDialog'>" + $(xml).find("html").html() + "<br/><div style='text-align:center'><button id='seleccionaMesa'>" + textoBoton + "</button></div></div>");
                      
                      $("#seleccionaMesa").button().click(function(){
                          //Aqui va el ajax que manda a llamar al webservice y recibe la respuesta
                          $.ajax({
                            url: "table_picker.jsp?$empresa=" + $("#_enterprise_").val() + "&$empleado=" + $("#_ce_").val() + "&" + $("#object_picker").serialize() + "&$ta=save",
                            dataType: ($.browser.msie) ? "text" : "xml",
                            success: function(data) {
                                if (typeof data == "string") {
                            ...
                            error: function(xhr, err) {
                                alert("Error al recuperar los datos de la mesa: \n" + +xhr.responseText);
                            }
                        })
