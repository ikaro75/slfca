...
( function($) {
        ...
        $.ajax(
        {
            ...
            success:  function(data){
                ...
                return hTags.each(function(){
                    ...
                    $("#"+ sTab).tabs({
                        tabTemplate:  $("#_cp_").val()==10?"<li><a href='#{href}'>#{label}</a></li>":"<li><a href='#{href}'>#{label}</a><span class='ui-icon ui-icon-close'>Cerrar tab</span></li>"
                    });
                                        
                    /*var selectedTab = $("#tab_1_0").tabs( "option", "selected" );
                    $("#tab_1_0 ul li a").eq(selectedTab).text("<span class='ui-button-icon-primary ui-icon ui-icon-gear'>Configuración</span>");*/
                    ...
                    $( "#"+sTab+" span.ui-icon-close" ).live( "click", function() {
                       ...
                       $("#"+TabId) .tabs( "remove", index );
                        ...
                        //Verifica si hay algún timer asociado y lo elimina
                        for (e=0; e<jsonConfig.gridTimers.length; e++) {
                            if (sTab.split("_")[2]==jsonConfig.gridTimers[e].gridSuffix.split("_")[1]) 
                                jsonConfig.gridTimers.splice(e,1);                            
                        }
                    });
                    ...
                    $(this).find(".menu")
                        ...
                        .click(function(e, data) {
                            ...
                            var requierePrefiltro = $(this).attr("prefiltro");
                            ...
                            if ($("#tab_"+nCategoria+"_"+nAplicacion+"_"+nEntidad+"_"+nPK).length) {
                                ...
                                $("#"+ sTab).tabs("select", "#tab_"+nCategoria+"_"+nAplicacion+"_"+nEntidad+"_"+nPK);
                                    ...
                                    if (nAplicacion==1) {
                                        $("#tab_1_0  a[href=#tab_1_1_2_0]").html("<span class='ui-button-icon-primary ui-icon ui-icon-gear'><span class='ui-button-text'>Configuración</span></span>");
                                    }
                                    ...
                                    $(oTabPanel).appportlet({
                                            ...
                                            register: "0",
                                            gridData: "",
                                            logPortlet:$("#_cp_").val()==10?0:1, //portlet de actividad reciente
                                            filterPortlet: $("#_cp_").val()==10?0:1, //portlets de filtros
                                            kardexPortlet: $("#_cp_").val()==10?0:1, //portlet de kardex
                                            templatePortlet:$("#_cp_").val()==10?0:1, //portlet de plantillas 
                                            reportPortlet:$("#_cp_").val()==10?0:1, //portlet de reportes
                                            requierePrefiltro: requierePrefiltro,
                                            tabTitle: sForma
                                    });                                    
                        ...
                        //Tuneando el botón del backend :-)
                        $("#showEntity_1_1_2 ").button( { icons: {primary:'ui-icon-gear'} } );
                        
                        if ($("#_cp_").val()==10) {
                            $("#tabMisAplicaciones").remove();
                            $( "a[href='#tabMisAplicaciones']").closest("li").remove();
                            $("#showEntity_1_129_613").click();
                            $("#appMenu_1_0").remove();
                        }    
                    });
            },
    ...
    $.fn.appmenu.handleMenu = function(xml, categoria){   
        ...
        $(xml).find("registro").each(function(){
            ...
            prefiltro = $(this).find("forma").attr("prefiltro")=="true"?"1":"0"; 
            sHtml+="<a href='#' class='menu' id='showEntity_" + nCategoria + "_" + nAplicacion + "_" + nEntidad +"' name='showEntity_" + nCategoria + "_" + nAplicacion + "_" + nEntidad +"' aplicacion_padre='" + nAplicacionPadre + "' aplicacion='"+nAplicacion+ "' forma='" + sForma +"' prefiltro='" + prefiltro + "'>" + sTituloAplicacion + "</a>";
