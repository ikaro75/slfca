	...
	function initialRender() {
		...
		content = $("<div class='fc-content' style='position:relative'/>")
			.prependTo(element);
                ...
                $("<div class='fc-content-paul' style='position:relative;'><div id='divgrid_1_101_0' class='queued_grids' app='1' form='101' wsParameters='' titulo='Mis pendientes' leyendas='Nueva actividad, Editar actividad' inDesktop='true' openKardex='false' /></div></div>").appendTo(element);	// agregador por Daniel Martínez 30/abr/2013
...
function Header(calendar, options) {
	...
	// funcion agregada por Daniel Martínez 30-5-2013
			...
			button
				.click(function() {
					if (!button.hasClass(tm + '-state-disabled')) {
						...
						/*$('.fc-content-paul').load('index.php?action=agenda', function() {
	...
	function hideAgendaOverviewButton() {
		...
		$('.fc-button-paul').removeClass('fc-state-active');
                $('.fc-button-paul').removeClass('ui-state-active');
		$('.fc-header-left').show();
	}
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							var button = $(
								...
								.click(function() {
									if (!button.hasClass(tm + '-state-disabled')) {
                                                                                hideAgendaOverviewButton();	// Agregado por Daniel Martínez 30-abr-2013
										buttonClick();
									}
								})
								.mousedown(function() {
							...
							prevButton = button;
                                ...
                                //Agregado por Daniel Martínez 30/abr/2013
                                if(!options.showAgendaButton) {
                                    if (prevButton) {
                                           // prevButton.addClass(tm + '-corner-right');
                                    }
                                }
...
function AgendaEventRenderer() {
	...
	function draggableSlotEvent(event, eventElement, timeElement) {
		...
		eventElement.draggable({
			...
			revertDuration: opt('dragRevertDuration'),
			start: function(ev, ui) {
                                //var ev = ev.originalEvent; /* added this line FIXME dmartine@ilce.edu.mx */
				trigger('eventDragStart', eventElement, event, ev, ui);
				hideEvents(event, eventElement);
