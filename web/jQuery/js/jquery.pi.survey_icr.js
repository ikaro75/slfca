...
( function($) {
    ...
    $.fn.survey.getGUI = function(obj){
        ...
        if ($.fn.survey.options.modo=="open") {
            sBotonera+="<input type='button' style='float: right;' class='formButton' id='btnSiguiente_" + surveySuffix +"' value='Siguiente' /><input type='button' style='float: right;' class='formButton' id='btnGuardar_" + surveySuffix +"' value='Cerrar y guardar sin finalizar' /><input type='button' style='float: right;' class='formButton' id='btnAnterior_" + surveySuffix +"' value='Anterior' /></div></td></tr></table></div>";
    ...
    $.fn.survey.setFormObjects = function(){  
        ...
        $.ajax(
        {   
            url: $.fn.survey.options.xmlUrl + "&$cfep="+ $.fn.survey.options.claveFormaEntidadParticipante + "&$ccp=" + $.fn.survey.options.pk + "&$pk=" + $.fn.survey.options.pk + "&$cp="+ $.fn.survey.options.claveProspecto+"&$ta=" + $.fn.survey.options.modo,
            ...
            success:  function(data){
                ...
                $("#btnAnterior_"+ surveySuffix+",#btnSiguiente_"+ surveySuffix + ",#btnGuardar_"+ surveySuffix).button().click(function() {
                    ...
                    else if ((this.id=='btnSiguiente_'+ surveySuffix))
                        document.getElementById('$hacia').value='adelante';
                    else if ((this.id=='btnGuardar_'+ surveySuffix))
                        document.getElementById('$hacia').value='guarda';
               ...
               $("#btn_ReenviaCodigoAutorizacion"+ surveySuffix).button().click(function() {
                ...
                if (($.fn.survey.options.seccionActual==$.fn.survey.options.ultimaSeccion /*&& $("#_cp_").val()== "2"*/)
                    || ( // o si el estatus de cuestionario=5 y el perfil es el de un auditor y
                        ...
                        $.fn.survey.options.respuestaCondicional_5_siEstaEsLaRespuesta=="true"  && $("#_cp_").val()== "3")) {
                            ...
                            $("#btnGuardar_"+ surveySuffix).button();
                             $("#btnGuardar_"+ surveySuffix).hide(); 
                ...
                /*$("input:radio").unbind("click").click(function(event) {
                ...
                $(".ocultarPreguntas").click(function() {
                    //Primero se revisa el cache para ver si no se ocultaron previamente otras preguntas
                    if ($("#_cache_").val()!="") {
                        aPreguntasAMostrar=$("#_cache_").val().split("=")[1].split(",");
                        for (i=0;i<aPreguntasAMostrar.length;i++) {
                              $("#pregunta_" + aPreguntasAMostrar[i]).parent().parent().show();
                              $("#pregunta_" + aPreguntasAMostrar[i]).parent().parent().next().show();
                        } 
                    }
                    
                    if ($(this).attr("ocultar_preguntas")!="" && $(this).attr("checked")=="checked") {
                        aPreguntasAOcultar=$(this).attr("ocultar_preguntas").split(",");
                        $("#_cache_").val("preguntasOcultadas="+$(this).attr("ocultar_preguntas"));
                        for (i=0;i<aPreguntasAOcultar.length;i++) {
                            $("#pregunta_" + aPreguntasAOcultar[i]).parent().parent().hide();
                            $("#pregunta_" + aPreguntasAOcultar[i]).parent().parent().next().hide();
                        }
                    }
                });
                    ...
                    $(jqForm[0]).find('.obligatorio').each(function() {
                        ...
                        if (this.id!="" && this.id!=undefined)  {
                            ...
                            if ($(this).attr("type")=="radio")  {
                                if (!$("input[name='" + this.id + "']:radio").is(':checked') && $(this).is(":visible")) {
                            ...
                            } else if ($.trim(this.value)=="" && $(this).attr("type")!="checkbox" && $(this).is(":visible")) {
                                ...
                                if ($(jqForm[0]).find("." + pregunta + ":checked").length==0 && $(this).is(":visible")) {
                                    $("#td_" + this.name).addClass("errorencampo");
                                    $(this).addClass("errorencampo");
                                    //$($($(this).parent().parent().children()[0]).children()[1]).show();
                                    bCompleto=false;                                                                
                ...
                function processXml(data) { 
                    ...
                    if (siguienteAccion=="continua") {
                    ...
                    } else if (siguienteAccion=="Cierra") {
                        ...
                        $("#dlgModal_"+ surveySuffix).dialog("destroy");
                        $("#dlgModal_"+ surveySuffix).remove();
                        alert('Cuestionario finalizado exitosamente');
                        //Aqui solo falta que se refresque el grid
                        $("#" +$.fn.survey.options.gridPorActualizar).jqGrid().trigger("reloadGrid");
                    }          
