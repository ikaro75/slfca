...
try {
    ...
    try {
            if (request.getParameter("page") != null) {
                pagina = Integer.parseInt(request.getParameter("page"))==0?1:Integer.parseInt(request.getParameter("page"));
            }
    } catch (Exception e) {
    ...
    tempForma = new Forma(new Consulta(forma, tipoAccion, String.valueOf(pk), w, user, rows, pagina, sidx.toString(), sord),false);
...
if( tempForma.getNumeroDeRegistros() >0 ) {
	total_paginas = Math.ceil(tempForma.getNumeroDeRegistros()/rows);
        if ((tempForma.getNumeroDeRegistros() % rows)>0)
                total_paginas+=1;
} else {
	total_paginas = 0;
}
...
<configuracion_forma>
    ...
    <total_paginas><%= total_paginas %></total_paginas>
    <registros><%=tempForma.getNumeroDeRegistros()%></registros>
    <busqueda_rapida><![CDATA[<%=tempForma.getBusquedaRapida()%>]]></busqueda_rapida>
</configuracion_forma>
<% 
